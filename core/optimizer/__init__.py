...
class DefaultOptimizerConstructor(object):
    ...
    def __init__(self, cfg):
        if not isinstance(cfg, dict):
            raise TypeError('optimizer_cfg should be a dict',
                            f'but got {type(cfg)}')
        self.op_name, self.optimizer_cfg = self.init_optimizer_cfg(cfg)
        self.paramwise_cfg = self.init_parawise_cfg(cfg)
        self.base_lr = self.optimizer_cfg.get('lr', None)
        self.base_wd = self.optimizer_cfg.get('weight_decay', None)
        self._validate_cfg()
        self.optimizers = self.init_optimizer_dict()[self.op_name]
        self.logger = LoggerManager().get_logger('Optimizer')
        self.logger(f"Using {self.op_name} optimizer, with {self.optimizer_cfg} config")

    @staticmethod
    def init_optimizer_cfg(cfg):
        opt_cfg = cfg.copy()
        opt_name = opt_cfg.pop('name')
        if 'paramwise_cfg' in opt_cfg:
            opt_cfg.pop('paramwise_cfg')
        return opt_name, opt_cfg

    @staticmethod
    def init_parawise_cfg(cfg):
        paramwise_cfg = cfg.get('paramwise_cfg', {})
        return paramwise_cfg

    @staticmethod
    def get_torch_optimizers():
        torch_optimizers_dict = {}
        for module_name in dir(torch.optim):
            ...
            if inspect.isclass(_optim) and issubclass(_optim,
                ...
                torch_optimizers_dict[module_name] = _optim
        return torch_optimizers_dict

    def init_optimizer_dict(self):
        torch_optimizers_dict = self.get_torch_optimizers()
        return torch_optimizers_dict
    ...
    def __call__(self, model):
        ...
        return self.optimizers(params, **self.optimizer_cfg)
