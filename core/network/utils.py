...
def hmdecoder(hms):
    # t, l, d, r = [hms[i] for i in range(len(hms))]
    height, width = hms[0].size()
    pts = torch.zeros((4, 2))
    for hm_counter, hm in enumerate(hms):
        pt_y, pt_x = get_top_yx(hm, height, width)
        pts[hm_counter] = torch.as_tensor([pt_x, pt_y])
    return pts
