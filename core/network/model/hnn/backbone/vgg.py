...
class VGG(nn.Module):
    ...
    def __init__(self, features, init_weights=True, channel=3):
...
def make_layers(cfg, batch_norm=False, channel=1):
    layers = []
    in_channels = channel
    module_list = []
    for v in cfg:
...
def vgg11(channel=1, pretrained=False, **kwargs):
    ...
    model = VGG(make_layers(cfg['A'], channel=channel), **kwargs)
    if pretrained:
        model.load_state_dict(torch.load(os.path.join(models_dir, model_name['vgg11'])))
    return model


def vgg11_bn(channel=1, pretrained=False, **kwargs):
    ...
    if pretrained:
        kwargs['init_weights'] = False
    model = VGG(make_layers(cfg['A'], batch_norm=True, channel=channel), **kwargs)
    if pretrained:
        model.load_state_dict(torch.load(os.path.join(models_dir, model_name['vgg11_bn'])))
    return model


def vgg13(channel=1, pretrained=False, **kwargs):
    ...
    if pretrained:
        kwargs['init_weights'] = False
    model = VGG(make_layers(cfg['B'], channel=channel), **kwargs)
    if pretrained:
        model.load_state_dict(torch.load(os.path.join(models_dir, model_name['vgg13'])))
    return model


def vgg13_bn(channel=1, pretrained=False, **kwargs):
    ...
    if pretrained:
        kwargs['init_weights'] = False
    model = VGG(make_layers(cfg['B'], batch_norm=True, channel=channel), **kwargs)
    if pretrained:
        model.load_state_dict(torch.load(os.path.join(models_dir, model_name['vgg13_bn'])))
    return model


def vgg16(channel=1, pretrained=False, **kwargs):
    ...
    if pretrained:
        kwargs['init_weights'] = False
    model = VGG(make_layers(cfg['D'], channel=channel), **kwargs)
    if pretrained:
        model.load_state_dict(torch.load(os.path.join(models_dir, model_name['vgg16'])))
    return model


def vgg16_bn(channel=1, pretrained=False, **kwargs):
    ...
    if pretrained:
        kwargs['init_weights'] = False
    model = VGG(make_layers(cfg['D'], batch_norm=True, channel=channel), **kwargs)
    if pretrained:
        model.load_state_dict(torch.load(os.path.join(models_dir, model_name['vgg16_bn'])))
    return model


def vgg19(channel=1, pretrained=False, **kwargs):
    ...
    if pretrained:
        kwargs['init_weights'] = False
    model = VGG(make_layers(cfg['E'], channel=channel), **kwargs)
    if pretrained:
        model.load_state_dict(torch.load(os.path.join(models_dir, model_name['vgg19'])))
    return model


def vgg19_bn(channel=1, pretrained=False, **kwargs):
    ...
    if pretrained:
        kwargs['init_weights'] = False
    model = VGG(make_layers(cfg['E'], batch_norm=True, channel=channel), **kwargs)
    if pretrained:
